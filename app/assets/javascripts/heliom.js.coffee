...
html.className += if Heliom.HAS_TRANSITIONS then ' has-transitions' else 'has-no-transitions'
html.className += if Heliom.HAS_SVG then ' has-svg' else ' has-no-svg'
html.className += if Heliom.IS_RETINA then ' retina' else 'non-retina'
...
html.className += ' chrome'     if /\bchrome\b/i.test navigator.userAgent
