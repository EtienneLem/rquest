...
# * Push to Heroku
desc 'bump, compile, commit, tag & deploy app'
...
task :bump do
  ...
  content = "module Rquest\n\s\sVERSION = '#{@new_version}'\nend\n"
  `echo "#{content}" > lib/rquest/version.rb`
...
task :tag do
  version = @new_version || Rquest::VERSION
  `git add lib/rquest/version.rb && git commit -m "Bump version to #{version}"`
...
namespace :assets do
  ...
  task :commit do
    puts "Removing #{Rquest::VERSION} assets"

    js_remove_path = "public/js/scripts-#{Rquest::VERSION}.min.js"
    js_heliom_remove_path = "public/js/heliom-#{Rquest::VERSION}.min.js"
    css_remove_path = "public/css/styles-#{Rquest::VERSION}.min.css"
